...
Additionally, quite a few challenges were faced in getting the `text` variable to append correctly to `info-holder`'s `<div>`. Using console.log(text) I managed to debug this, and I decided to treat the `<div>` as a list, using appendChild(newText) to add the text correctly. 
